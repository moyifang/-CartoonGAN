...
class CartoonGAN(object):
    ...
    def discriminator(self, x_init, reuse=False, scope="discriminator"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            for i in range(1, self.n_dis):
                x = conv(x, channel * 2, kernel=3, stride=2, pad=1, use_bias=True, sn=self.sn,
                         scope='conv_s2_' + str(i))
                x = lrelu(x, 0.2)

                x = conv(x, channel * 4, kernel=3, stride=1, pad=1, use_bias=False, sn=self.sn,
    ...
    def build_model(self):
        ...
        self.test_real_A = tf.placeholder(tf.float32, [1, self.img_size, self.img_size, self.img_ch],
                                          name='test_real_A')
        ...
        if self.gan_type.__contains__('gp') or self.gan_type.__contains__('lp') or self.gan_type.__contains__('dragan'):
            GP = self.gradient_panalty(real=self.real_B, fake=self.fake_B) + self.gradient_panalty(self.real_B,
                                                                                                   fake=self.real_B_smooth)
        else:
            GP = 0.0

        v_loss = self.vgg_weight * vgg_loss(self.real_A, self.fake_B)
        g_loss = self.adv_weight * generator_loss(self.gan_type, fake_B_logit)
        d_loss = self.adv_weight * discriminator_loss(self.gan_type, real_B_logit, fake_B_logit,
                                                      real_B_smooth_logit) + GP
        ...
        self.init_optim = tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999).minimize(self.Vgg_loss,
                                                                                           var_list=G_vars)
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                if epoch < self.init_epoch:
                    ...
                    print("Epoch: [%3d] [%5d/%5d] time: %4.4f v_loss: %.8f" % (
                    epoch, idx, self.iteration, time.time() - start_time, v_loss))
